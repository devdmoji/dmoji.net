  ...
  $business_type.on('change', function() {
	...
	if($('.main-processor').length > 0){
		...
		$main_processor.find('.btn-go-inc').on('click', function(e) {
			$main_processor.find('.mpcs-box').addClass('on');
			$main_processor.find('.step-s').removeClass('on');
			$main_processor.find('.step-or').removeClass('on');
			e.preventDefault();
		});
		...
		var processor = function(e){
			...
			var cond1 = !url || !url.trim() ? ' - 웹사이트 주소 입력\n' : '';
			var cond2 = !bt ? ' - 업종 선택\n' : '';
			var cond3 = !had ? ' - 광고 운영 여부 선택\n' : '';
